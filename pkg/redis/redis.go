...
*/
func (rc *Conn) Hgetall(key string) ([]Field, error) {

	resultMap, err := rc.client.HGetAll(key).Result()
	if err == rd.Nil {
		return nil, nil
	}
	if err != nil {
	...
	if resultMap != nil {
		...
		for key, value := range resultMap {
			fields = append(fields, Field{Field: key, Value: value})
		}
		return fields, nil
	}
...
// 获取有序集合的成员数
func (rc *Conn) ZRem(key string,members ...interface{}) (int64, error) {
	return  rc.client.ZRem(key,members...).Result()
}
...
func (rc *Conn) ZRange(key string, start,stop int64) ([]string, error) {
	val, err := rc.client.ZRange(key, start,stop).Result()
	if err == rd.Nil {
		return nil, nil
	}
	return val, err
